...
import {
  ...
  CloudSnow, CloudLightning, Eye, EyeOff, Droplets, Loader2,
} from 'lucide-react';
...
const PublicMapBottom = ({
  ...
  const tabs: { id: Tab; label: string; icon: React.ReactNode }[] = [
    ...(hasRoute ? [{ id: 'elevation' as Tab, label: 'Elevation', icon: <Mountain className="w-4 h-4" /> }] : []),
    { id: 'legend', label: 'Legend', icon: <CircleDot className="w-4 h-4" /> },
    ...(tempRange ? [{ id: 'weather' as Tab, label: tempRange, icon: <Cloud className="w-4 h-4" /> }] : []),
  ];
