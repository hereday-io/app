      ...
      event.routes.forEach((route) => {
        ...
        if (!map.getLayer(`route-line-${route.id}`)) {
          map.addLayer({
            ...
            type: 'line',
            source: `route-${route.id}`,
            paint: { 'line-color': route.color, 'line-width': 3, 'line-opacity': hiddenRouteIds.has(route.id) ? 0 : 0.45, 'line-dasharray': [2, 2] },
  ...
  useEffect(() => {
    ...
    const renderMarkers = () => {
      ...
      event.pois.forEach(poi => {
        ...
        const el = document.createElement('div');
        el.style.cssText = `display:flex;align-items:center;justify-content:center;width:36px;height:36px;border-radius:50%;background:white;border:2px solid ${tone.dot};font-size:18px;cursor:pointer;box-shadow:0 2px 8px rgba(0,0,0,0.15);opacity:${isHighlighted ? 1 : 0.25};transition:opacity 0.2s,transform 0.15s;`;
