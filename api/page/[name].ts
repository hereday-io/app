...
function injectMetaTags(html: string, meta: PageMeta, name: string): string {
  ...
  // Strip any existing og:*, twitter:*, and canonical tags from the
  // shell so we don't duplicate them when our block lands.
  out = out.replace(
  ...
  // Inject the per-page block right after the description tag.
  out = out.replace(
    /(<meta name="description"[^>]*\/?>)/,
    `$1\n    ${metaBlock}`,
  );
...
export default async function handler(
  ...
  // Unknown route falls through to the unmodified shell. Should never
  // happen because vercel.json only routes specific names here, but
  // staying defensive keeps a typo from bricking the page.
  ...
  res.setHeader('Content-Type', 'text/html; charset=utf-8');
  // Marketing copy changes rarely — long edge cache, modest browser cache.
  // stale-while-revalidate keeps the experience fast even when copy changes.
  res.setHeader(
    'Cache-Control',
